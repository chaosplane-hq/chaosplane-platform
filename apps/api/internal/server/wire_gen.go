...
func InitializeServer(ctx context.Context) (*Server, error) {
	...
	authService := service.NewAuthService(pool, cfg, rdb, emailService)
	...
	invitationService := service.NewInvitationService(pool, emailService)
	...
	notificationService := service.NewNotificationService(pool, emailService)
	...
	resultAnalysisService := service.NewResultAnalysisService(pool, llmClient)
	resultAnalysisHandler := handler.NewResultAnalysisHandler(resultAnalysisService)
	aiChatService := service.NewAIChatService(pool, llmClient)
	aiChatHandler := handler.NewAIChatHandler(aiChatService)
	samlService := service.NewSAMLService(pool, cfg)
	...
	emailChangeService := service.NewEmailChangeService(pool, emailService)
